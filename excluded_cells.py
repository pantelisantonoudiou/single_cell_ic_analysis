...
if __name__ == '__main__':
    ...
    # find cell ids with bad responses
    ...
    # fast spiking
    fast_spiking = set(df_basic_io[df_basic_io['max_firing_rate'] > 50]['cell_id'])
    
    # find common cells to exclude
    ...
    for stim, metric in zip(stim_types, metrics):
        ...
        # Exclude fast spiking and cells with bad responses
        df = df[~df['cell_id'].isin(to_exclude)]

        # counts
        counts = df.groupby(['cell_id', 'treatment']).mean(numeric_only=True).groupby('treatment').count()['freq']
